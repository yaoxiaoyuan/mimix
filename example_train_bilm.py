...
class BiLMDataset(SimpleDataset):
    """
    """
    def __init__(self, 
                 data_dir,
...
def main(model_config, train_config):
    ...
    train_dataset = BiLMDataset(train_dir, batch_size, symbol2id, mask_rate, device)
    val_dataset = None
    if train_config.get("val_dir", None) is not None:
        val_dir = train_config["val_dir"]
        test_batch_size = train_config["test_batch_size"]
        val_dataset = BiLMDataset(val_dir, test_batch_size, symbol2id, mask_rate, device)
    test_dataset = None
    if train_config.get("test_dir", None) is not None:
        test_dir = train_config["test_dir"]
        test_batch_size = train_config["test_batch_size"]
        test_dataset = BiLMDataset(test_dir, test_batch_size, symbol2id, mask_rate, device)  
...
def run_train():
    """
    """
    usage = "usage: exapmle_train_bilm.py --model_conf <file> --train_conf <file>"
